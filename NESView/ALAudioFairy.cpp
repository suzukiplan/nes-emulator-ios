...
static void* sound_thread(void* context)
{
    ...
    while (c->alive) {
        ...
        if (st < BUFNUM) {
            ...
            while (static_cast<void>(alGetSourcei(c->sndASrc, AL_BUFFERS_PROCESSED, &st)), st == 0) {
                usleep(1000);
            }
...
void ALAudioFairy::lock()
{
    pthread_mutex_lock(&mutex);
}

void ALAudioFairy::unlock()
{
...
void ALAudioFairy::buffering(void* b, int* size)
{
    ...
    // バッファリングしたサイズが小さすぎるとptptするので 1k (2KB) は溜まるまで待つ
